...
def run_parser(
    ...
    for index, row in enumerate(match_rows, start=1):
        ...
        try:
            ...
            failed += 1
            print(f"{prefix} failed: {exc} | saved={saved} skipped={skipped} rejected={rejected} failed={failed}", flush=True)
            continue
        ...
        if not is_valid:
            ...
            rejected += 1
            print(f"{prefix} rejected={reason} | saved={saved} skipped={skipped} rejected={rejected} failed={failed}", flush=True)
            time.sleep(delay)
            continue
        ...
        saved += 1
        print(f"{prefix} saved | saved={saved} skipped={skipped} rejected={rejected} failed={failed}", flush=True)
        time.sleep(delay)
