...
from src.ml.add_team_priority_features import build_full_team_priority_acc, team_priority_features_for_teams
from src.ml.build_player_stats import build_player_hero_stats_for_frame, build_player_stats_for_frame
from src.ml.role_utils import build_role_lookup, candidate_role_features, load_hero_roles, roster_candidate_role_fit
...
def load_player_stats(patch_label="7.41"):
    ...
    patch_players_path = Path(f"data/patch_{patch_num}/players.parquet")
    patch_players = pd.read_parquet(patch_players_path) if patch_players_path.exists() else pd.DataFrame()

    alltime_players = pd.DataFrame()
    alltime_player_paths = sorted(Path("data").glob("patch_*/players.parquet"))
    if alltime_player_paths:
        frames = []
        for path in alltime_player_paths:
            try:
                frames.append(pd.read_parquet(path))
            except Exception:
                pass
        if frames:
            alltime_players = pd.concat(frames, ignore_index=True)

    data = {}
    for key, path in paths.items():
        if path.exists():
            data[key] = pd.read_parquet(path)
        elif key == "patch_player" and not patch_players.empty:
            data[key] = build_player_stats_for_frame(patch_players)
        elif key == "patch_player_hero" and not patch_players.empty:
            data[key] = build_player_hero_stats_for_frame(patch_players)
        elif key == "alltime_player" and not alltime_players.empty:
            data[key] = build_player_stats_for_frame(alltime_players)
        elif key == "alltime_player_hero" and not alltime_players.empty:
            data[key] = build_player_hero_stats_for_frame(alltime_players)
        else:
...
@st.cache_data(show_spinner=False)
def load_candidate_reference(patch_label="7.41", dataset="players_team_role", action_type="pick", features=()):
    _, _, _, ml_dir, _ = get_patch_paths(patch_label)
    suffix = "" if dataset == "base" else f"_{dataset}"
    path = ml_dir / f"draft_candidates_{action_type}{suffix}.parquet"
    if not path.exists():
        return pd.DataFrame()

    columns = ["candidate_hero_id"]
    columns.extend(col for col in features if col != "candidate_hero_id")
    try:
    ...
    if not numeric_cols:
        return pd.DataFrame(columns=["candidate_hero_id"])
    return df.groupby("candidate_hero_id")[list(numeric_cols)].mean(numeric_only=True).reset_index()


@st.cache_resource(show_spinner=False)
def load_model(patch_label="7.41", dataset="players_team_role", action_type="pick"):
...
@st.cache_data(show_spinner=False)
def load_feature_config(patch_label="7.41", dataset="players_team_role", action_type="pick"):
    _, _, _, _, model_dir = get_patch_paths(patch_label)
...
@st.cache_data(show_spinner=False)
def load_team_priority_accs(patch_label="7.41"):
    return build_full_team_priority_acc(patch_label)


@st.cache_data(show_spinner=False)
def load_role_lookup():
    return build_role_lookup(load_hero_roles())
...
def _role_features_from_rosters(hero_id, ally_picks_before, enemy_picks_before, own_roster, opponent_roster, lookups, role_lookup):
    out = candidate_role_features(hero_id, ally_picks_before, enemy_picks_before, role_lookup)
    own_fit = roster_candidate_role_fit(own_roster, hero_id, role_lookup, lookups.get("alltime_player", {}))
    opponent_fit = roster_candidate_role_fit(opponent_roster, hero_id, role_lookup, lookups.get("alltime_player", {}))
    out["own_best_player_candidate_role_fit"] = own_fit["best_player_candidate_role_fit"]
    out["own_mean_player_candidate_role_fit"] = own_fit["mean_player_candidate_role_fit"]
    out["opponent_best_player_candidate_role_fit"] = opponent_fit["best_player_candidate_role_fit"]
    out["opponent_mean_player_candidate_role_fit"] = opponent_fit["mean_player_candidate_role_fit"]
    return out


def _reference_defaults(patch_label, action_type, dataset, features):
    means = load_candidate_reference(
...
def recommend(
    ...
    ref = _reference_defaults(patch_label, action_type, dataset, features)
    ...
    team_priority_accs = load_team_priority_accs(patch_label) if dataset in {"players_team", "players_team_role"} else None
    role_lookup = load_role_lookup() if dataset in {"interactions_role", "players_team_role"} else None
    team_priority_feature_fn = None
    if team_priority_accs is not None:
        team_priority_feature_fn = lambda hero_id: team_priority_features_for_teams(
            hero_id,
            acting_team_id,
            opponent_team_id,
            team_priority_accs[0],
            team_priority_accs[1],
        )
    role_feature_fn = None
    if role_lookup is not None:
        role_feature_fn = lambda hero_id: _role_features_from_rosters(
            hero_id,
            ally_picks_before,
            enemy_picks_before,
            own_roster,
    ...
    df = build_recommendation_rows(
        ...
        team_priority_feature_fn=team_priority_feature_fn,
        role_feature_fn=role_feature_fn,
    )
    df = prepare_model_frame(df, features, cat_features).copy()
    ...
    present_debug_cols = [col for col in debug_cols if col in df.columns]
    return df[output_cols + explanation_cols + present_debug_cols], own_roster, opponent_roster
