...
def run_data_update(
    ...
    total = int(fetch) + int(convert) + (3 if build_ml else 0) + 1
    progress = UpdateProgress(total)
    if fetch:
        progress.step("fetch missing OpenDota JSON")
        run_parser(
            patch_label=patch_label,
            limit=limit,
    ...
    if convert:
        progress.step("convert raw JSON to parquet")
        run_conversion(
            patch_label=patch_label,
            rebuild=rebuild,
            validate=True,
