...
REPORT_DATASETS = ["base", "interactions", "interactions_role", "players_team", "players_team_role"]
...
def _write_comparison(report_dirs, metrics_df):
    ...
    for action in ["pick", "ban"]:
        pairs = [
            ("base", "interactions"),
            ("interactions", "interactions_role"),
            ("interactions", "players_team"),
            ("players_team", "players_team_role"),
        ]
...
def _write_importance_markdown(report_dirs):
    ...
    for path in sorted(report_dirs["importance"].glob("*_feature_importance.csv")):
        stem = path.name.replace("_feature_importance.csv", "")
        if stem not in active_stems:
            continue
        df = pd.read_csv(path).head(30)
        df.insert(0, "model", stem)
        frames.append(df)
    if not frames:
        return
